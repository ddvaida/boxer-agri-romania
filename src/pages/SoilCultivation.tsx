...
import { soilCultivationProducts } from '@/data/allProducts';

const SoilCultivation: React.FC = () => {
  ...
  const brands = [...new Set(soilCultivationProducts.map(product => product.brand))];
  const productTypes = [...new Set(soilCultivationProducts.map(product => {
    if (product.name.includes('Freză')) return 'Freze Rotative';
    if (product.name.includes('Grapă')) return 'Grape Rotative';
    if (product.name.includes('Cultivator')) return 'Cultivatoare';
    return 'Alte utilaje';
  }))];

  // Product categories with stats
  const categories = [
    { name: 'Freze Rotative', count: soilCultivationProducts.filter(p => p.name.includes('Freză')).length, description: 'Pentru pregătirea primară a solului' },
    { name: 'Grape Rotative', count: soilCultivationProducts.filter(p => p.name.includes('Grapă')).length, description: 'Pentru finisarea solului' },
    { name: 'Cultivatoare', count: soilCultivationProducts.filter(p => p.name.includes('Cultivator')).length, description: 'Pentru prelucrarea secundară' }
  ];

  // Filter and search products
  const filteredProducts = soilCultivationProducts
    .filter(product => {
      const matchesBrand = filterBrand === 'all' || product.brand === filterBrand;
      const matchesType = filterType === 'all' || 
        (filterType === 'Freze Rotative' && product.name.includes('Freză')) ||
        (filterType === 'Grape Rotative' && product.name.includes('Grapă')) ||
        (filterType === 'Cultivatoare' && product.name.includes('Cultivator'));
  ...
  return (
    <div className="min-h-screen bg-gradient-section">
      ...
      <div className="bg-primary/5 py-4">
        <div className="container mx-auto px-4">
          <div className="flex items-center space-x-2 text-sm text-muted-foreground">
            ...
            <span>/</span>
            <span className="text-foreground font-medium">Prelucrarea Solului</span>
          </div>
        </div>
      </div>
      ...
      <section className="py-12 bg-gradient-hero text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-bold mb-6">Prelucrarea Solului</h1>
            <p className="text-xl mb-8 opacity-90">
              Utilaje profesionale pentru pregătirea eficientă a solului - freze rotative, grape și cultivatoare de înaltă calitate
            </p>
            
            {/* Key Statistics */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
                <div className="text-3xl font-bold text-secondary">{soilCultivationProducts.length}+</div>
                <div className="text-white/80">Modele disponibile</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
                <div className="text-3xl font-bold text-secondary">{brands.length}</div>
                <div className="text-white/80">Branduri de încredere</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
                <div className="text-3xl font-bold text-secondary">15+</div>
                <div className="text-white/80">Ani de experiență</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-8">Categorii de Produse</h2>
            ...
            {categories.map((category, index) => (
              <Card key={index} className="hover:shadow-elegant transition-all duration-300 cursor-pointer group">
                <CardContent className="p-6 text-center">
                  ...
                  <div className="text-2xl font-bold text-primary">{category.count} modele</div>
                </CardContent>
              </Card>
            ))}
          </div>
                  ...
                  <CardContent className={`p-6 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                    ...
                    <div className="space-y-2 mb-4 text-sm">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Lățime de lucru:</span>
...
export default SoilCultivation;