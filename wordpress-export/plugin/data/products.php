<?php
/**
 * Date produse pentru WordPress
 */

return array(
    // Produse Boxer AGF Series
    array(
        'id' => 'boxer-agf120',
        'name' => 'Boxer AGF120 Cositor cu Braț',
        'nameEn' => 'Boxer AGF120 Arm Mower',
        'description' => 'Cositor cu braț hydraulic profesional pentru întreținerea marginilor drumurilor, șanțurilor și zonelor greu accesibile.',
        'brand' => 'Boxer',
        'category' => 'agricultural-machinery',
        'subcategory' => 'grassland-cultivation',
        'priceFrom' => 4200,
        'priceRange' => '4.200 - 4.800 EUR',
        'specifications' => array(
            'Lățimea de lucru (cm)' => 120,
            'Raza de acțiune (m)' => 4.2,
            'Unghiul de înclinare (grade)' => 90,
            'Diametrul maxim tăiat (mm)' => 60,
            'Greutate (kg)' => 285
        ),
        'images' => array(
            'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1'
        ),
        'mainImage' => 'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1',
        'features' => array(
            'Braț hydraulic articulat cu 3 secțiuni',
            'Sistem anti-rupere cu siguranțe',
            'Control complet din cabina tractorului',
            'Lame schimbabile rapid',
            'Protecție împotriva pietrelor'
        ),
        'technicalData' => array(
            'Putere necesară (CP)' => '35-50',
            'Viteză PTO (RPM)' => 540,
            'Tip atașare' => 'Trei puncte Cat I',
            'Presiune hidraulică (bar)' => 180,
            'Consum hidraulic (L/min)' => 30
        ),
        'availability' => 'in-stock',
        'isFeatured' => true,
        'tags' => array('cositor', 'braț', 'hydraulic', 'margini'),
        'weight' => 285
    ),
    
    array(
        'id' => 'boxer-agf140',
        'name' => 'Boxer AGF140 Cositor cu Braț',
        'nameEn' => 'Boxer AGF140 Arm Mower',
        'description' => 'Cositor cu braț de 140cm pentru lucrări intensive de întreținere a vegetației pe margini de drumuri și în zonele dificile.',
        'brand' => 'Boxer',
        'category' => 'agricultural-machinery',
        'subcategory' => 'grassland-cultivation',
        'priceFrom' => 4650,
        'priceRange' => '4.650 - 5.300 EUR',
        'specifications' => array(
            'Lățimea de lucru (cm)' => 140,
            'Raza de acțiune (m)' => 4.5,
            'Unghiul de înclinare (grade)' => 90,
            'Diametrul maxim tăiat (mm)' => 70,
            'Greutate (kg)' => 310
        ),
        'images' => array(
            'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1'
        ),
        'mainImage' => 'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1',
        'features' => array(
            'Braț hydraulic robust',
            'Sistem de siguranță avansat',
            'Lame cu protecție antișoc',
            'Control hidraulic precis'
        ),
        'technicalData' => array(
            'Putere necesară (CP)' => '40-60',
            'Viteză PTO (RPM)' => 540,
            'Tip atașare' => 'Trei puncte Cat I/II',
            'Presiune hidraulică (bar)' => 180,
            'Consum hidraulic (L/min)' => 35
        ),
        'availability' => 'in-stock',
        'tags' => array('cositor', 'braț', '140cm', 'professional'),
        'weight' => 310
    )
    
    // COSITORI CU DISCURI - DM SERIES
    array(
        'id' => 'boxer-dm120',
        'name' => 'Boxer DM120 Cositor cu Discuri',
        'nameEn' => 'Boxer DM120 Disc Mower',
        'description' => 'Cositor cu discuri de 120cm pentru cosirea eficientă a furajelor. Echipat cu discuri de înaltă calitate pentru o tăiere precisă.',
        'brand' => 'Boxer',
        'category' => 'agricultural-machinery',
        'subcategory' => 'grassland-cultivation',
        'priceFrom' => 3800,
        'priceRange' => '3.800 - 4.200 EUR',
        'specifications' => array(
            'Lățimea de lucru (cm)' => 120,
            'Numărul de discuri' => 3,
            'Diametrul discurilor (mm)' => 165,
            'Înălțimea de tăiere (mm)' => '15-70',
            'Greutate (kg)' => 180
        ),
        'images' => array(
            'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1'
        ),
        'mainImage' => 'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1',
        'features' => array(
            'Discuri cu unghi de atac optimizat',
            'Sistem de protecție împotriva pietrelor',
            'Bară de tăiere robustă',
            'Întreținere minimă'
        ),
        'technicalData' => array(
            'Putere necesară (CP)' => '25-40',
            'Viteză PTO (RPM)' => 540,
            'Tip atașare' => 'Trei puncte Cat I',
            'Viteză de lucru (km/h)' => '8-15'
        ),
        'availability' => 'in-stock',
        'tags' => array('cositor', 'discuri', 'furaje', 'iarba'),
        'weight' => 180
    ),

    // COSITORI CU CIOCANELE - FM SERIES  
    array(
        'id' => 'boxer-fm120',
        'name' => 'Boxer FM120 Cositor cu Ciocanele',
        'nameEn' => 'Boxer FM120 Flail Mower',
        'description' => 'Cositor cu ciocanele pentru vegetație dură și zone accidentate. Ideal pentru terenuri inegale și vegetație înaltă.',
        'brand' => 'Boxer',
        'category' => 'agricultural-machinery', 
        'subcategory' => 'grassland-cultivation',
        'priceFrom' => 4500,
        'priceRange' => '4.500 - 5.100 EUR',
        'specifications' => array(
            'Lățimea de lucru (cm)' => 120,
            'Numărul de ciocanele' => 36,
            'Diametrul rotorului (mm)' => 220,
            'Înălțimea de tăiere (mm)' => '20-100',
            'Greutate (kg)' => 290
        ),
        'images' => array(
            'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1'
        ),
        'mainImage' => 'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1',
        'features' => array(
            'Ciocanele din oțel de calitate superioară',
            'Sistem de siguranță prin șuruburi de ruptură',
            'Carcasă robustă pentru condiții grele',
            'Reglare ușoară a înălțimii de tăiere'
        ),
        'technicalData' => array(
            'Putere necesară (CP)' => '35-55',
            'Viteză PTO (RPM)' => 540,
            'Tip atașare' => 'Trei puncte Cat I/II',
            'Viteză de lucru (km/h)' => '5-12'
        ),
        'availability' => 'in-stock',
        'tags' => array('cositor', 'ciocanele', 'flail', 'terenuri dificile'),
        'weight' => 290
    ),

    // MULCERE FORESTIERĂ - HM SERIES
    array(
        'id' => 'boxer-hm180',
        'name' => 'Boxer HM180 Mulcer Heavy-Duty',
        'nameEn' => 'Boxer HM180 Heavy-Duty Mulcher',
        'description' => 'Mulcer profesional pentru aplicații forestiere și întreținerea vegetației dure. Potrivit pentru crengi și arbuști.',
        'brand' => 'Boxer',
        'category' => 'forestry-machinery',
        'subcategory' => 'mulchers',
        'priceFrom' => 8500,
        'priceRange' => '8.500 - 9.800 EUR',
        'specifications' => array(
            'Lățimea de lucru (cm)' => 180,
            'Diametrul maxim procesat (mm)' => 120,
            'Numărul de ciocanele' => 48,
            'Tip ciocanele' => 'Y-Blade',
            'Greutate (kg)' => 650
        ),
        'images' => array(
            'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1'
        ),
        'mainImage' => 'https://www.heustractors.com/uploads/media/product_list/02/1872-AGF220-6.png?v=1-1',
        'features' => array(
            'Rotor masiv pentru aplicații grele',
            'Ciocanele interschimbabile tip Y',
            'Protecție hidraulică la suprasarcină',
            'Carcasă întărită pentru durabilitate maximă'
        ),
        'technicalData' => array(
            'Putere necesară (CP)' => '80-120',
            'Viteză PTO (RPM)' => 1000,
            'Tip atașare' => 'Trei puncte Cat II',
            'Viteză de lucru (km/h)' => '3-8'
        ),
        'availability' => 'in-stock',
        'isFeatured' => true,
        'tags' => array('mulcer', 'forestier', 'heavy-duty', 'crengi'),
        'weight' => 650
    )

    // Adaugă restul produselor aici...
);